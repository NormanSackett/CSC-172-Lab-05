...
public class InfixCalc {
	...
	public static void main(String[] args) { //sets up GUI and adds action to the button
	...
	public static URQueue<Character> infixToPostfix(char[] input) { //performs infix-to-postfix algorithm
		...
		for (int i = 0; i < input.length; i++) {
			...
			else if (c == ')') { // if a closed parenthesis, pop from the stack until the closed parenthesis is hit
				while (stack.size() > 0 && stack.peek() != '(') outputQueue.enqueue(c);
				
				stack.pop();
			}
			else if (c != ' ') { // if an operator, push to the stack in the correct priority location (spaces are ignored)
				stack.push(c);
			}
		}
	...
	public static int getPriority(Character c) {
		...
		// highest priority gets the smallest return value
		if (c == '^') return 1;
		if (c == '!') return 2;
		if (c == '/' || c == '*' || c == '%') return 3;
		if (c == '-' || c == '+') return 4;
